...
- **提示词分层 = 缓存前缀**：系统层只放逐回合稳定的文本（角色 / 规则 / 规范 / 工具说明）；**故事书草稿快照、实体 id 目录、本轮引用目标一律作为请求的最后一条消息**下发。供应商的上下文缓存是**前缀缓存**——草稿每采纳一次建议、每跑一步工具都会变，放进系统层就等于让「系统层 + 全部历史」整段失配，每一步都按原价重算整段会话。
...
- **给模型**：发送时前端把引用解析成 `focus`（**完整实体定义**），后端把它与草稿快照一起、作为请求的**最后一条消息**下发（不进系统层：见 §4.3 的缓存前缀说明）。模型不再猜，拿到的是现状全貌；并被告知「目标之外的改动要明确说明」。
...
## 6. 自动上下文压缩（派生 surface）

模型的会话（`ai_conversations` / `RigProvider` 内存里的会话 entry）是**派生 surface**：权威日志 `commands` 永不改写。所以压缩**只重写 surface**——玩家看到的叙事、回放与存档都不受影响，不存在「早期剧情被丢掉」。语义与默认参数对齐 DSH 的 `compaction-basic`。

- **触发**：① `pressure` —— 供应商回传的真实 `input_tokens`（加上本轮新输入的估算）越过 `models[].ctx × ai.compaction.threshold_ratio`（默认 0.8）；② `context_overflow` —— 供应商报上下文超限时，做一次更激进的压缩并重试该请求（每轮最多一次）。没声明 `ctx` 的模型不做压力压缩，只留兜底。
- **选区**：从尾巴往前按 token 预算定价，逐字保留最近 `ctx × retain_ratio`（默认 0.16）；切点吸附到**回合开头**，不劈开一回合内的续轮往返；兜底压缩也至少保留最近 2 个回合。没得压（历史本身就是不可分单元）就什么都不做、也不写任何东西。
- **摘要**：请求 = `[原系统提示词, ...被遮蔽的原文..., user(压缩指令)]`——逐字重放上一次请求的真前缀，只追加尾部指令，**复用供应商的热缓存**；`temperature = 0.3`，输出上限 `ai.compaction.max_tokens`（默认 8192）。摘要必须比原文更短，否则放弃本次压缩。
- **落地**：被遮蔽的那段换成**一条** user 消息（`<已压缩摘要>` 框住摘要），后面接着原样保留的尾巴，写回 `ai_conversations`；游玩页日志落一条 `System` 事件（code = `context_compacted`）。
- **配置**：`ai.compaction = { enabled, threshold_ratio, retain_ratio, max_tokens }`；窗口取 `providers[].models[].ctx`（目录快照里就有，落盘到 config.json 后端才看得见）。
- **结对（C 范式）同样在服务端压**：前端照旧发**全量展示历史**，后端按 `pair_threads.compaction_json` 里的检查点裁成 `[稳定 system, 检查点?, ...尾巴..., 本轮草稿快照]`。检查点只在重新压缩时变（逐字回放），前缀缓存才稳；指纹对不上（清空 / 换线程 / 改过历史）立即作废重压。触发、保留比例、摘要复用热前缀、超限兜底重试都与游玩页同款；压缩指令是 `pair.compaction`。权威展示历史 `pair_messages` **永不改写**——压缩不会让创作者丢掉任何一条对话。

> 为什么不每回合砍头：前缀缓存的队首一变，后面整段按原价重算——`octopus.db` 的 `ai_call` 事件实测命中率从 ~98% 掉到 0.4%。压缩的代价被摊销到「几十轮一次」，且阈值给到窗口的 80%，正常游玩根本不会触发。
...
| Q23 | 自动上下文压缩 | 只重写派生 surface（权威日志 / 展示历史不动）· 窗口取 `models[].ctx` · 阈值 0.8 / 保留 0.16（DSH 同款）· 摘要请求复用热前缀 · 超限兜底压一次再重试 · 摘要没变短就放弃 · 游玩页压 `ai_conversations`、结对压 `pair_threads.compaction_json`（前端只发全量历史） |
